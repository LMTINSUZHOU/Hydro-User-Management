...
- 点击“修改”后弹出表单，可修改用户名、电子邮件、密码、显示名、学校、学号、角色、Groups、权限。
...
为了保证“修改”和“删除”按钮在表格最后一列完整显示，学校、学号、Groups、权限默认不在查询表格中展示；这些字段仍可在修改弹窗中查看和修改，也会包含在导出 CSV 中。
...
密码字段留空表示不修改当前密码。填写完成后点击“保存”。保存成功后会回到当前查询结果。
...
在查询结果的最后一列点击“删除”，页面会弹出确认框。确认后插件会执行安全删除逻辑。超级管理员和系统保护用户会显示禁用的“修改/删除”按钮，不能直接操作：
